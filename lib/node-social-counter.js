(function() {
  ...
  record = function(params) {
    ...
    if(isNaN(nid)){
      console.log('No nid found for url: "'+ target_url +'"'); 
    ...
    console.log(key+': '+ (nowTimestamp - requestsCompleted[key]));
  ...
  getCompletedKeys = function(){
    ...
    for (var key in requestQueue){
      if (requestQueue.hasOwnProperty(key)) {
        ...
        console.log('^^^^^^');
          //requestsCompleted[key]['last_processed'] = Math.round(new Date().getTime() / 1000);
    ...
    /*for (var key in requestsCompleted){
      if(requestsCompleted.hasOwnProperty(key)){
        console.log('vvvvvv');
        console.log(requestsCompleted.key.last_processed);
        console.log('^^^^^^');
      }
    }*/
