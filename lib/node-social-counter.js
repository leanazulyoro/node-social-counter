(function() {
  ...
  getFacebookCount = function(key, target_url){
    ...
    if(typeof requestsInProgress[key].facebook === 'undefined' ){
      requestsInProgress[key].fb_hits || (requestsInProgress[key].fb_hits = 0);
      requestsInProgress[key].fb_hits += 1;
      log('key ' + key + ' hits = ' + requestsInProgress[key].fb_hits, 'DEBUG');


      if(requestsInProgress[key].fb_hits >= config.facebook.hits_required) {

        requestsInProgress[key].facebook = Math.round(new Date().getTime() / 1000);
        requestQueue[key].fb_count = 'inprogress';
        ...
        var req = request.get(
          ...
          timeout: config.facebook.timeout*1000,
            ...
            if (err) { // request error handling
              ...
              requestQueue[key].fb_count = 'failed';
            } else {
              ...
              if(typeof body.shares !== 'undefined'){
                ...
                delete requestsInProgress[key].fb_hits; // clear hits
  ...
  getTwitterCount = function(key, target_url){
    ...
    if(typeof requestsInProgress[key].twitter === 'undefined'){
      ...
      requestQueue[key].tw_count = 'inprogress';
      ...
      var req = request.get(
        ...
        timeout: config.twitter.timeout*1000,
          ...
          if (err) { // request error handling
            ...
            requestQueue[key].tw_count = 'failed';
          } else {
  ...
  getGoogleCount = function(key, target_url){
    if(typeof requestsInProgress[key].google === 'undefined'){
      ...
      requestQueue[key].g_count = 'inprogress';
      ...
      request.post({
        ...
        body:'[{"method":"pos.plusones.get","id":"p","params":{"nolog":true,"id":"'+target_url+'","source":"widget","userId":"@viewer","groupId":"@self"},"jsonrpc":"2.0","key":"p","apiVersion":"v1"}]'
          ...
          if (err) { // request error handling
            ...
            requestQueue[key].g_count = 'failed';
          } else {
            body = JSON.parse(body);
  ...
  getCompletedKeys = function(){
    ...
    for (var key in requestQueue){
      if (requestQueue.hasOwnProperty(key)) {
        ...
        } else if (requestQueue[key].fb_count == 'failed'){
          delete requestQueue[key].fb_count;     
        }
        ...
        } else if (requestQueue[key].tw_count == 'failed'){
          delete requestQueue[key].tw_count;     
        }
        ...
        } else if (requestQueue[key].g_count == 'failed'){
          delete requestQueue[key].g_count;     
        }
    ...
    for (var key in requestsInProgress){
      if (requestsInProgress.hasOwnProperty(key)) {
        ...
        if(typeof requestsInProgress[key].facebook === 'undefined' && 
          ...
          typeof requestsInProgress[key].fb_hits === 'undefined'){
  ...
  flush = function() {
    ...
    log("Request in Progress size: " + ObjectLength(requestsInProgress), 'DEBUG');
      ...
      request.post({
        ...
        form: data
          ...
          if (err) { 
            ...
            log('FLUSHED - ' + JSON.stringify(data));
          }
        }
      ).setMaxListeners(0);
