(function() {
  ...
  record = function(params) {
    ...
    if(!_.isUndefined(config.safe_hosts)) {
    ...
    if(_.isUndefined(requestsInProgress[key])) {
      requestsInProgress[key] = {};
      requestsInProgress[key].hits = 0;
    }

    if(_.isUndefined(requestQueue[key])) {
    // add to queue
      requestQueue[key] = {};
    }
    if(!_.isUndefined(requestQueue[key])) {
  ...
  if(_.isUndefined(requestsInProgress[key].facebook) && requestsInProgress[key].hits > config.facebook.fb_hits_required){
      ...
      var req = request.get(
        ...
        timeout: config.facebook.fb_timeout*1000,
          ...
          if (err) { // request error handling
            ...
            if(!_.isUndefined(body.shares)){
              requestQueue[key].fb_count = parseInt(body.shares);

            } else if (!_.isUndefined(body.error)){ // facebook's error handling
  ...
  getTwitterCount = function(key, target_url){
    ...
    if(_.isUndefined(requestsInProgress[key].twitter)){
  ...
  getGoogleCount = function(key, target_url){
    if(_.isUndefined(requestsInProgress[key].google)){
      ...
      request.post({
        ...
        body:'[{"method":"pos.plusones.get","id":"p","params":{"nolog":true,"id":"'+target_url+'","source":"widget","userId":"@viewer","groupId":"@self"},"jsonrpc":"2.0","key":"p","apiVersion":"v1"}]'
          ...
          if (err) { // request error handling
            ...
            body = JSON.parse(body);
            if(!_.isUndefined(body[0].result.metadata.globalCounts.count) ){
  ...
  getCompletedKeys = function(){
    ...
    for (var key in requestQueue){
      if (requestQueue.hasOwnProperty(key)) {
        ...
        if(_.isUndefined(requestQueue[key].fb_count) && 
           _.isUndefined(requestQueue[key].tw_count) && 
           _.isUndefined(requestQueue[key].g_count) ){
  ...
  clearExpiredKeys = function(){
    ...
    for (var key in requestsInProgress){
      if (requestsInProgress.hasOwnProperty(key)) {
        ...
        if(_.isUndefined(requestsInProgress[key].facebook) && 
           _.isUndefined(requestsInProgress[key].twitter) && 
           _.isUndefined(requestsInProgress[key].google) && 
           _.isUndefined(requestsInProgress[key].hits) ){
