(function() {
  ...
  if(typeof requestsInProgress[key].facebook === 'undefined' && requestsInProgress[key].hits > config.facebook.fb_hits_required){
      ...
      facebookQueue[priority].push({key:target_url});      
      ...
      if (facebookQueue.hasOwnProperty(priority)) {
        ...
        for(var key in facebookQueue[priority]){
          if (facebookQueue[priority].hasOwnProperty(key)) {
            console.log(facebookQueue[priority][key]);
